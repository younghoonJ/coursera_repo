...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> intermediateRDD = sites
                .join(ranks)
                .flatMapToPair(
                        kv -> {
        ...
        // comeplete operator chaining will `throw error: incompatible types: incompatible parameter types in lambda expression has some problem` at submission
        return intermediateRDD.reduceByKey(Double::sum).mapValues(v -> 0.15 + 0.85 * v);
    }
}
